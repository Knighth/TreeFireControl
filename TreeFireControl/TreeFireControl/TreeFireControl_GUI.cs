...
using VD = TreeFireControl.SomeModName_VehicleData.GoodsType; 

namespace TreeFireControl
{
    public class TreeFireControlGUI : UIPanel
    {
        ...
        public static TreeFireControlGUI instance;  //incase we or someone else needs wants to interact with us they can 'get' a reference to us.
        ...
        {
            base.Start();
            if (Mod.DEBUG_LOG_ON & Mod.DEBUG_LOG_LEVEL > 0) Logger.dbgLog(string.Concat("Attempting to create our display panel.  ",DateTime.Now.ToString(DTMilli).ToString()));
            this.size = new Vector2(WIDTH, HEIGHT);
            this.backgroundSprite = "MenuPanel";
            this.canFocus = true;
            this.isInteractive = true;
            this.BringToFront();
            this.relativePosition = new Vector3((TreeFireControl_Loader.parentGuiView.fixedWidth / 2) - 200, (TreeFireControl_Loader.parentGuiView.fixedHeight / 2) - 350);
            this.opacity = Mod.config.GuiOpacity;
            this.cachedName = cacheName;
            TreeFireControlGUI.instance = this;
            ...
            if (Mod.DEBUG_LOG_ON) Logger.dbgLog(string.Concat("Display panel created. ",DateTime.Now.ToString(DTMilli).ToString()));
        }
        ...
        private void DoOnStartup()
        {
            ...
            if (m_AutoRefreshCheckbox.isChecked)
            {
                ...
                if (Mod.DEBUG_LOG_ON) { Logger.dbgLog("RefreshDisplayDataWrapper coroutine started."); }
            }
            else
            {
        ...
        private void CreateTextLabels() 
        {
            ...
            m_AutoRefreshCheckbox.text = string.Concat("Use AutoRefresh  (", Mod.config.AutoRefreshSeconds.ToString("f1"), " sec)");
            ...
            m_AutoRefreshCheckbox.isChecked = Mod.config.GuiUseAutoUpdate;
        ...
        {
            if (Mod.DEBUG_LOG_ON) { Logger.dbgLog("AutoRefreshButton was toggled to: " + bValue.ToString()); }
            Mod.config.GuiUseAutoUpdate = bValue;  //update our config object.
            TreeFireControl.Configuration.Serialize(Mod.MOD_CONFIGPATH,Mod.config); //write change to disk.
            if (bValue == true)
            {
                if (!CoDisplayRefreshEnabled) { this.StartCoroutine(RefreshDisplayDataWrapper());}
                if (Mod.DEBUG_LOG_ON) { Logger.dbgLog("Starting all coroutines that were not already started, bValue=" + 
                    bValue.ToString()); }
            }
            else
            {
                ...
                if (Mod.DEBUG_LOG_ON) { Logger.dbgLog("Stopping all coroutines: " + bValue.ToString()); }
            }
            return;
        }
        ...
        private IEnumerator RefreshDisplayDataWrapper() 
        {
            if (CoDisplayRefreshEnabled == true)
            {
                ...
                if (Mod.DEBUG_LOG_ON & Mod.DEBUG_LOG_LEVEL > 0) Logger.dbgLog("Refresh vehicleData* coroutine exited; Only one allowed at a time.");
            ...
            if (Mod.DEBUG_LOG_ON & Mod.DEBUG_LOG_LEVEL > 0) Logger.dbgLog("Refresh vehicleData coroutine exited due to AutoRefresh disabled, visiblity change, or already refreshing.");
            yield break;
        }
        ...
        private void RefreshDisplayData()
        {
            ...
            try
            {
                ...
                if (Mod.DEBUG_LOG_ON & Mod.DEBUG_LOG_LEVEL >= 3) Logger.dbgLog("Refreshing display data completed. " + DateTime.Now.ToString(DTMilli));
            }
            catch (Exception ex)
            {
                isRefreshing = false;
